use crate::{Error, Plugin, PluginBuilder, PluginId, PluginType, VERSION, store::PluginStore};
...
macro_rules! create_task_registry {
	($plugin_name:expr, $task_builder:expr) => {
		puniyu_task::TaskRegistry { plugin_name: $plugin_name, builder: $task_builder.into() }
	};
}
...
impl PluginRegistry {
			...
			PluginType::Path(path) => {
				...
				unsafe {
					...
					let tasks: Vec<_> = plugin_builder
						...
						.into_iter()
						.map(|task_builder| {
							TaskManager::add_task(create_task_registry!(plugin_name, task_builder))
						})
						.collect();
	...
	pub fn get_all_plugins() -> Vec<Plugin> {
		PLUGIN_STORE.get_all_plugins().into_values().map(Plugin::from).collect()
	}
}
