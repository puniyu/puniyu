...
impl<'h> HookRegistry {
	pub fn register(source: SourceType, hook: Arc<dyn Hook>) -> Result<u64, Error> {
		let hook = HookInfo { source, builder: hook };
		STORE.insert(hook)
	}
	pub fn unregister<H>(hook: H) -> Result<(), Error>
	where
		H: Into<HookId<'h>>,
	{
		let hook = hook.into();
	...
	pub fn unregister_with_hook_name(name: &str) -> Result<(), Error> {
		let raw = STORE.raw();
		let mut map = raw.write().expect("Failed to acquire lock");
