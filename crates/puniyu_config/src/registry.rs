...
///
/// ```rust,ignore
/// use puniyu_config::registry::ConfigRegistry;
/// use puniyu_config::types::ConfigInfo;
/// use std::path::PathBuf;
///
/// // 注册配置
/// let config = ConfigInfo {
///     path: PathBuf::from("config/plugin.toml"),
///     value: toml::toml! {
///         enabled = true
///         timeout = 30
///     },
...
///
/// // 通过路径查询
/// if let Some(value) = ConfigRegistry::get(PathBuf::from("config/plugin.toml")) {
///     println!("配置: {:?}", value);
/// }
///
/// // 更新配置
...
impl ConfigRegistry {
	...
	/// # 参数
	///
	/// - `id`: 配置标识符，可以是 `u64` 索引或 `PathBuf` 路径
	///
	/// # 返回值
	///
	...
	///
	/// // 通过路径获取
	/// let value = ConfigRegistry::get(PathBuf::from("config/app.toml"));
	/// ```
	...
	///
	/// // 通过路径更新
	/// ConfigRegistry::update(PathBuf::from("config/app.toml"), new_value)?;
	/// ```
