...
fn parses_positional_args() {
	...
	let args = result.into_args(&hello_positional_args()).unwrap();
...
fn parses_positional_ints() {
	...
	let args = result.into_args(&calc_args()).unwrap();
...
fn parses_long_named_arg() {
	...
	let args = result.into_args(&hello_named_args()).unwrap();
	assert_eq!(args["name"][0].as_str(), Some("Alice"));
}
...
fn collects_duplicate_positional_as_vec() {
	...
	let args = result.into_args(&args).unwrap();
...
fn errors_on_missing_required() {
	let result = default_parser().parse("hello").unwrap().into_args(&hello_named_args());
	assert!(matches!(result, Err(Error::MissingRequired { .. })));
}

#[test]
fn errors_on_invalid_int() {
	let result = default_parser().parse("calc abc 2").unwrap().into_args(&calc_args());
	assert!(matches!(result, Err(Error::InvalidValue { .. })));
}

#[test]
fn errors_on_unknown_named_arg() {
	let result = default_parser().parse("hello --name Alice --unknown val").unwrap().into_args(&hello_named_args());
	assert!(matches!(result, Err(Error::UnknownArgument { .. })));
}

#[test]
fn errors_on_too_many_positional() {
	let result = default_parser().parse("hello a b").unwrap().into_args(&hello_named_args());
