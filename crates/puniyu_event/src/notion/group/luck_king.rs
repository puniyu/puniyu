...
pub struct GroupLuckKing<'n> {
	...
	contact: &'n Contact<'n>,
	sender: &'n Sender<'n>,
	content: &'n GroupLuckKingType,
}

impl<'n> GroupLuckKing<'n> {
	pub fn new(builder: NotionBuilder<'n, Contact<'n>, Sender<'n>, GroupLuckKingType>) -> Self {
		Self {
			bot: builder.bot,
...
impl<'n> EventBase for GroupLuckKing<'n> {
	...
	fn sub_event(&self) -> &NotionSubEventType {
		&NotionSubEventType::GroupLuckKing
	}
...
impl NotionBase<GroupLuckKingType> for GroupLuckKing<'_> {
	fn notion(&self) -> &str {
		"收到群运气王事件"
	}

	fn content(&self) -> &GroupLuckKingType {
		self.content
	}
}
