...
pub struct GroupHonorChange<'n> {
	...
	contact: &'n Contact<'n>,
	sender: &'n Sender<'n>,
	content: &'n GroupHonorChangeType,
}

impl<'n> GroupHonorChange<'n> {
	pub fn new(builder: NotionBuilder<'n, Contact<'n>, Sender<'n>, GroupHonorChangeType>) -> Self {
		Self {
			bot: builder.bot,
...
impl<'n> EventBase for GroupHonorChange<'n> {
	...
	fn sub_event(&self) -> &NotionSubEventType {
		&NotionSubEventType::GroupHonorChange
	}
...
impl NotionBase<GroupHonorChangeType> for GroupHonorChange<'_> {
	fn notion(&self) -> &str {
		"收到群聊荣誉变更事件"
	}

	fn content(&self) -> &GroupHonorChangeType {
		self.content
	}
}
