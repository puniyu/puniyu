...
pub struct GroupMemberTitleChange<'n> {
	...
	contact: &'n Contact<'n>,
	sender: &'n Sender<'n>,
	content: &'n GroupMemberTitleChangeType,
}

impl<'n> GroupMemberTitleChange<'n> {
	pub fn new(
		builder: NotionBuilder<'n, Contact<'n>, Sender<'n>, GroupMemberTitleChangeType>,
	) -> Self {
		Self {
			bot: builder.bot,
...
impl<'n> EventBase for GroupMemberTitleChange<'n> {
	...
	fn sub_event(&self) -> &NotionSubEventType {
		&NotionSubEventType::GroupMemberTitleChange
	}
...
impl NotionBase<GroupMemberTitleChangeType> for GroupMemberTitleChange<'_> {
	fn notion(&self) -> &str {
		"收到群成员头衔变动事件"
	}

	fn content(&self) -> &GroupMemberTitleChangeType {
		self.content
	}
}
