...
pub struct GroupCardChange<'n> {
	...
	contact: &'n Contact<'n>,
	sender: &'n Sender<'n>,
	content: &'n GroupCardChangeType,
}

impl<'n> GroupCardChange<'n> {
	pub fn new(builder: NotionBuilder<'n, Contact<'n>, Sender<'n>, GroupCardChangeType>) -> Self {
		Self {
			bot: builder.bot,
...
impl<'n> EventBase for GroupCardChange<'n> {
	...
	fn sub_event(&self) -> &NotionSubEventType {
		&NotionSubEventType::GroupCardChange
	}
...
impl NotionBase<GroupCardChangeType> for GroupCardChange<'_> {
	fn notion(&self) -> &str {
		"收到群成员名片变动事件"
	}

	fn content(&self) -> &GroupCardChangeType {
		self.content
	}
}
