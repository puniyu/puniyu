use super::{Element, ElementType, RawMessage, TextMessage};
...
impl MusicElement {
	...
	pub fn qq(mut self) -> Self {
		if let MusicData::Standard { id, .. } = self.data {
			self.data =
				MusicData::Standard { platform: MusicPlatform::QQ.to_platform_string(), id };
		}
		self
	}
	...
	pub fn net_ease(mut self) -> Self {
		if let MusicData::Standard { id, .. } = self.data {
			self.data =
				MusicData::Standard { platform: MusicPlatform::NetEase.to_platform_string(), id };
		}
		self
	}
	...
	pub fn migu(mut self) -> Self {
		if let MusicData::Standard { id, .. } = self.data {
			self.data =
				MusicData::Standard { platform: MusicPlatform::MiGu.to_platform_string(), id };
		}
		self
	}
	...
	pub fn kugou(mut self) -> Self {
		if let MusicData::Standard { id, .. } = self.data {
			self.data =
				MusicData::Standard { platform: MusicPlatform::KuGou.to_platform_string(), id };
		}
		self
	}
	...
	pub fn kuwo(mut self) -> Self {
		if let MusicData::Standard { id, .. } = self.data {
			self.data =
				MusicData::Standard { platform: MusicPlatform::Kuwo.to_platform_string(), id };
		}
		self
	}
...
impl RawMessage for MusicElement {
	fn raw(&self) -> String {
		format!("[music:{:?}]", self.data)
	}
}
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
